...
private:
    ...
    Chest(const std::string& message, bool hstuff) : has_stuff(hstuff) {
...
public:
    ...
    Chest<F> map_stuff(std::function<F(const T&)> func) {
        ...
        return Chest<F>::sign(this->content.message);
    }

    /// If the chest has stuff returns default, otherwise applies a function on the stuff in the chest and returns it
    template<typename F>
    F map_or(const F& default, std::function<F(const T&)> func) {
        return this->has_stuff ? func(this->content.stuff) : default;
    }

    /// Maps on the stuff in the chest and returns it with func, otherwise calls fallback function default and returns that
    template<typename F>
    F map_or_fallback(std::function<F(const std::string&)> default, std::function<F(const T&)> func) {
        return this->has_stuff ? func(this->content.stuff) : default(this->content.message);
    }
...
Chest<T>::Chest(const Chest<T>& rhs) : has_stuff(rhs.has_stuff) {
    if (rhs.has_stuff) {
        this->content.stuff = rhs.content.stuff;
    } else {
        this->content.message = rhs.content.message;
    }
}
